...
<div class="widget-body">
                                <table class="table table-striped table-bordered table-advance table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th class="hidden-phone">Blog Image</th>
                                  ...
                                  @foreach($all_blog as $v_blog)
                                    <tr>
                                        <td><a href="#">{{$v_blog->blog_title}}</a></td>
                                        <td>{{$v_blog->author_name}}</td>
                                        <td><img src="{{URL::to($v_blog->blog_image)}}" width="60px" height="60px"/></td>
                                        <td>{{$v_blog->publication_status}}</td>
                                        <td>
                                        ...
                                        <td>
                                          @if($v_blog->publication_status == 1)
                                            <a href="{{URL::to('/unpublished_blog/'.$v_blog->blog_id)}}" class="btn btn-danger"><i class="icon-thumbs-down"></i></a>
                                         @else
                                            <a href="{{URL::to('/published_blog/'.$v_blog->blog_id)}}" class="btn btn-success"><i class="icon-thumbs-up"></i></a>
                                         @endif
